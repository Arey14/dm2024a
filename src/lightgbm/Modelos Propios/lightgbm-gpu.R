...
# paquetes necesarios para la Optimización Bayesiana
require("DiceKriging")
require("mlrMBO")
...
# Verificar si el directorio "./work" existe, si no, crearlo
if (!dir.exists("./work")) {
  dir.create("./work")
}
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  param_basicos <- list(
    ...
    seed = ksemilla_azar1,
    device = "gpu", # Habilitar GPU
    gpu_platform_id = 0, # ID de la plataforma de GPU (ajustar si es necesario)
    gpu_device_id = 0  # ID del dispositivo GPU (ajustar si es necesario)
  )
...
# defino donde voy a guardar el resultado de la Bayesian Optimization
kbayesiana <- paste0("./work/", PARAM$experimento, ".RDATA")

# donde voy a guardar las salidas
klog <- paste0("./work/", PARAM$experimento, ".txt")
klog_mejor <- paste0("./work/", PARAM$experimento, "_mejor.txt")
