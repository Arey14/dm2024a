...
sets = read.csv("~/Desktop/ITBA/Mineria de Datos/datasets/azarosos2.csv")
for (i in 1:5) {
  # cambiar aqui los parametros
  PARAM <- list()
  PARAM$minsplit <- sets[i,1]
  PARAM$minbucket <- sets[i,2]
  PARAM$maxdepth <- sets[i,3]
...
cat( "Train gan max: ", dataset[fold==1, max(ganancia_acumulada)], " ", sets[i,1],sets[i,2],sets[i,3],"\n" )
